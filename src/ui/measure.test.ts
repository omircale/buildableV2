...
describe('part dimensions shown when a part is isolated', () => {
  ...
  it('for every component of every catalog item, the drawn size equals its cut-list size', () => {
    for (const type of FURNITURE_TYPES.filter((f) => f.available)) {
      ...
      for (const c of model.components) {
        ...
        if (c.endCutDeg) {
          // A board with angled ends is ordered longer than the body it occupies: the angle is cut off
          // it at home. Its two other sides still have to match the board ordered exactly.
          expect(cut[2], `${type.kind} ${c.id} ordered board must be longer than its body`).toBeGreaterThan(drawn[2]);
          for (let i = 0; i < 2; i++) expect(drawn[i], `${type.kind} ${c.id}`).toBeCloseTo(cut[i], 6);
          continue;
        }
  ...
  it('the texts drawn on an isolated part are exactly its own sizes and cut-list size', () => {
    const words = { width: 'W', height: 'H', depth: 'D', thickness: 'T', part: 'Cut', cm: 'cm', inFrame: 'in frame' };
    for (const type of FURNITURE_TYPES.filter((f) => f.available)) {
      const { model } = runDesign(presetFor(type, { w: null, h: null, d: null })!);
      for (const c of model.components) {
        ...
        const inFrame = c.endCutDeg ? ` (in frame ${formatCm(Math.max(c.size.x, c.size.y, c.size.z))} cm)` : '';
        expect(l.cut).toBe(part ? `${part.id} · Cut: ${formatCm(part.lengthMm)} × ${formatCm(part.widthMm)} × ${formatCm(part.thicknessMm)} cm${inFrame}` : undefined);
      }
    }
  });
});
